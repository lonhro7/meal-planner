...
# Rich hand-tuned method rewrites (title -> [steps]), authored by review agents.
import os as _os
_OV_PATH = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "method_overrides.json")
_OVERRIDES = json.load(open(_OV_PATH, encoding="utf-8")) if _os.path.exists(_OV_PATH) else {}

# Words that don't help identify which step an ingredient is used in.
_STOP = set(("the and of with for fresh ground tinned dried baby extra light plain free range reduced fat "
             "large small medium whole grated chopped finely roughly sliced diced minced crushed thinly "
             "red green white brown boneless bone skinless mild hot sweet").split())
def _keywords(ing):
    kws = []
    for k in (ing.get("meat_cut"), ing.get("meat_type")):
        if k: kws.append(k.lower())
    for w in _re.findall(r"[a-z]+", ing["name"].lower()):
        if len(w) > 2 and w not in _STOP: kws.append(w)
    return kws
def _assign_steps(recipe):
    lower = [s.lower() for s in recipe["method_steps"]]
    for ing in recipe["ingredients"]:
        kws, idx = _keywords(ing), 0
        for si, text in enumerate(lower):
            if any(kw in text for kw in kws): idx = si; break
        ing["step_index"] = idx
def build_methods():
    used = 0
    for r in R:
        if r["title"] in _OVERRIDES:
            r["method_steps"] = list(_OVERRIDES[r["title"]]); used += 1
        else:
            newsteps = []
            for st in r["method_steps"]:
                newsteps.extend(_split_step(st))
            r["method_steps"] = newsteps
        _assign_steps(r)   # re-attach each ingredient to the step that first uses it
    print(f"method overrides applied: {used}/{len(R)}")
build_methods()
